...
export function handleError(error: any, alertService: CoreAlertService, customMessage: string): Observable<never> {
  ...
  if (error?.error?.errors?.length > 0) {
    ...
    alertService.error(`Error en el campo "${fieldName}": ${errorMessage}`);
  }
